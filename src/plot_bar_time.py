...
medians = {treatment: {budget: np.median(times) for budget, times in budgets.items()} for treatment, budgets in data.items()}

# Create the plot
budgets = sorted(set(budget for treatment_budgets in data.values() for budget in treatment_budgets))
